...
def match_with_gaps(my_word, other_word):
    ...
    returns: boolean, True if all the actual letters of my_word match the 
        ...
        False otherwise: 
    '''
    # first, remove the spaces between any un-guessed characters
    temp = ''
    for c in my_word:
        temp += c.strip()
    # check whether temp & other_word have the same length
    if len(temp) == len(other_word):
        # the lengths are the same
        # now test whether the revealed characters match
        match = True
        i = 0
        while match and i < len(temp):
            if temp[i] != other_word[i] and temp[i] != '_':
                match = False
            elif temp[i] == '_' and other_word[i] in temp:
                match = False
            else:
                i+=1
        if match:
...
def show_possible_matches(my_word):
    ...
    '''
    # create an emptry string to hold the results
    possible_matches = ''
    # loop through all words in wordlist
    for word in wordlist:
        # pass my_word and current word in wordlist to match_with_gaps
        if match_with_gaps(my_word, word):
            # if the current word is a potential match, add it to the string
            possible_matches += word + ' '
    # If any possible matches were found, return them through a print statement
    if len(possible_matches) > 0:
        return print('possible matches:\n',possible_matches)
    # If no possible matches were found, let the user know the bad news
    else:
        return print('Sorry, I could not find any possible matches.')
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while not is_word_guessed(secret_word, letters_guessed) and num_guesses!=0:
        ...
        if guess in secret_word:
            print('Good guess!:',
                  get_guessed_word(secret_word, letters_guessed))
            print('Input * to show all possible matches')
            print('-'*8)
        elif guess == '*' and len(letters_guessed) > 0:
